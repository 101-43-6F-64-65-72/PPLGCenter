...
export default function EmailOtpVerificationModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto bg-slate-900/50 backdrop-blur-xs animate-in fade-in duration-150 font-sans">
      <div className="relative w-full max-w-md bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden my-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 px-3.5 text-sm font-semibold text-slate-900 outline-none transition focus:bg-white focus:border-[#2C1EE8]"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-4 bg-[#2C1EE8] hover:bg-blue-700 disabled:opacity-50 text-white font-bold text-sm rounded-xl transition cursor-pointer flex items-center justify-center gap-2"
              >
                {isLoading ? (
                  <>
          ...
          {step === "challenge" && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between text-xs text-slate-500">
                <span>Kode verifikasi dikirim ke: <strong className="text-slate-800">{email}</strong></span>
                <button
                  type="button"
                ...
                <div className="flex items-center justify-between">
                  ...
                  {selectedSlots.some((s) => s !== null) && (
                    <button
                      type="button"
                      onClick={handleReset}
                      className="text-xs font-medium text-slate-400 hover:text-slate-700 flex items-center gap-1 transition"
                    >
                  ...
                  {selectedSlots.map((slot, index) => {
                    ...
                    return (
                      <div
                        ...
                        className={`h-16 rounded-xl border-2 transition-all flex flex-col items-center justify-center p-1.5 text-center select-none ${
                          isFilled
                ...
                {CURATED_TECH_OPTIONS.map((tech) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 px-1 rounded-xl border transition-all text-center relative cursor-pointer ${
                        isSelected
                          ? "border-[#2C1EE8] bg-indigo-50/80 font-extrabold text-[#2C1EE8]"
                          : "border-slate-200 bg-white hover:bg-slate-50 text-slate-800 font-bold"
                      }`}
                    >
                      {isSelected && (
                        <div className="absolute -top-1.5 -right-1.5 w-4 h-4 rounded-full bg-[#2C1EE8] text-white text-[9px] font-extrabold flex items-center justify-center">
                          {slotIndex + 1}
                        </div>
                      )}
              ...
              <div className="space-y-2 pt-1">
                <button
                  ...
                  className="w-full py-2.5 px-4 bg-[#2C1EE8] hover:bg-blue-700 disabled:opacity-40 text-white font-bold text-sm rounded-xl transition cursor-pointer flex items-center justify-center gap-2"
                >
                  {isLoading ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
          ...
          {step === "success" && (
            <div className="text-center py-4 space-y-4">
              ...
              <button
                type="button"
                onClick={onClose}
                className="w-full py-2.5 px-4 bg-[#2C1EE8] hover:bg-blue-700 text-white font-bold text-sm rounded-xl transition cursor-pointer"
              >
                Selesai
              </button>
            </div>
