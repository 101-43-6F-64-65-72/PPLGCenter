...
export default function OsisFacilityTab() {
  ...
  const getStatusBadge = (status) => {
    ...
    if (status.includes("Sarpras") || status === "Disetujui Sarpras") {
      return (
  ...
  const handleUpdateStatus = (bookingId, newStatus) => {
    facilityService.updateBookingStatus(bookingId, newStatus, note).catch((err) => {
      console.warn("Async booking status update warning:", err);
    });

    setBookings((prev) =>
      prev.map((b) =>
        b.id === bookingId ? { ...b, status: newStatus, verificator: "Pengurus OSIS" } : b
      )
    );
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Cari organisasi, kegiatan, atau fasilitas..."
            value={searchQuery}
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-lg font-black text-gray-900 flex items-center gap-2">
            ...
            <span>Peminjaman Fasilitas & Barang ({filteredBookings.length})</span>
          </h3>
          <span className="text-xs text-gray-500 font-medium">Monitoring Real-time OSIS</span>
        </div>
          ...
          {filteredBookings.map((b) => (
            <div key={b.id} className="p-5 hover:bg-gray-50/80 transition-colors space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                ...
                <span className="text-xs text-gray-400">Tgl Kegiatan: {b.date}</span>
              </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <button
                  ...
                  className="self-start md:self-center inline-flex items-center gap-1.5 px-4 py-2 rounded-xl text-xs font-bold bg-blue-50 text-[#2c1ee8] hover:bg-blue-100 transition-colors cursor-pointer"
                >
                  <Eye className="w-4 h-4" />
                  <span>Detail & Verifikasi OSIS</span>
                </button>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-xl rounded-3xl p-6 space-y-5 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="pt-2 flex items-center justify-end gap-2">
              <button
                onClick={() => handleUpdateStatus(selectedBooking.id, "Ditolak OSIS")}
