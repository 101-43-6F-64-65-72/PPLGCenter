...
export default function NotificationItem({ notification, onMarkRead, onDelete }) {
    ...
    switch (priority) {
      case 3:
        return {
          ...
          badge: (
            <span className="text-[9.5px] bg-rose-50 text-rose-700 border border-rose-200 font-bold font-mono px-1.5 py-0.2 rounded-none flex items-center gap-1 shrink-0 uppercase">
              <AlertTriangle className="w-3 h-3 text-rose-600" />
              Darurat
            </span>
          ),
          unreadBg: "bg-rose-50/20 hover:bg-rose-50/40",
      ...
      case 2:
        return {
          ...
          badge: (
            <span className="text-[9.5px] bg-amber-50 text-amber-800 border border-amber-200 font-bold font-mono px-1.5 py-0.2 rounded-none flex items-center gap-1 shrink-0 uppercase">
              <AlertCircle className="w-3 h-3 text-amber-600" />
              Penting
            </span>
          ),
          unreadBg: "bg-amber-50/15 hover:bg-amber-50/30",
      ...
      case 0:
        return {
          ...
          badge: (
            <span className="text-[9.5px] bg-slate-100 text-slate-600 border border-slate-200 font-bold font-mono px-1.5 py-0.2 rounded-none shrink-0 uppercase">
              Rendah
            </span>
          ),
          unreadBg: "bg-slate-50/50 hover:bg-slate-100/60",
      ...
      default:
        return {
          border: "border-l-4 border-l-[#2C1EE8]",
          badge: (
            <span className="text-[9.5px] bg-blue-50 text-[#2C1EE8] border border-blue-200 font-bold font-mono px-1.5 py-0.2 rounded-none shrink-0 uppercase">
              Normal
            </span>
          ),
          unreadBg: "bg-blue-50/10 hover:bg-blue-50/20",
          iconBg: "bg-blue-50 text-[#2C1EE8] border-blue-200",
          unreadDot: "bg-[#2C1EE8]",
          titleColor: "text-slate-900 font-bold",
        };
    }
  ...
  const content = (
    <div
      onClick={handleClick}
      className={`p-3.5 rounded-none border border-slate-200 transition-colors flex items-start gap-3 cursor-pointer ${priorityConfig.border} ${
        isRead
          ? "bg-white text-slate-700 hover:bg-slate-50 shadow-xs"
          : `${priorityConfig.unreadBg} text-slate-900 shadow-xs`
      }`}
    >
      <div className={`shrink-0 w-8 h-8 rounded-none border flex items-center justify-center transition-colors ${
      ...
      <div className="flex-1 min-w-0 overflow-hidden">
        <div className="flex items-center justify-between gap-2 mb-0.5 min-w-0">
          <h4 className={`text-xs sm:text-sm truncate uppercase min-w-0 flex-1 ${!isRead ? priorityConfig.titleColor : "text-slate-900 font-bold"}`}>
            {stripHtml(notification.title)}
          </h4>
          <div className="flex items-center gap-1.5 shrink-0">
            {priorityConfig.badge}
            {!isRead && <span className={`w-2 h-2 rounded-none ${priorityConfig.unreadDot}`} />}
          </div>
        </div>
        ...
        <span className="text-[10px] font-mono text-slate-400 font-medium mt-1 block">{formattedTime}</span>
      </div>
      ...
      <div className="flex flex-col gap-1 shrink-0">
        {!isRead && onMarkRead && (
          <button
            ...
            title="Tandai Dibaca"
            className="p-1 text-slate-400 hover:text-slate-900 hover:bg-slate-100 rounded-none transition-colors cursor-pointer"
          >
