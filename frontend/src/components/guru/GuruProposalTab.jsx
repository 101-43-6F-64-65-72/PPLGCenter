...
export default function GuruProposalTab() {
  ...
  const handleUpdateStatus = (proposalId, newStatus) => {
    proposalService.updateProposalStatus(proposalId, newStatus, teacherNote).catch((err) => {
  ...
  const getStatusBadge = (status) => {
    ...
    if (status.includes("Disetujui") || status === "ACC Proposal") {
      return (
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder="Cari proposal atau organisasi binaan..."
            value={searchQuery}
        ...
        <div className="flex items-center gap-2 text-xs font-extrabold text-[#2c1ee8] bg-blue-50 px-4 py-2 rounded-2xl border border-blue-100">
          ...
          <span>Wewenang Verifikasi: Guru Pembina & Waka Kesiswaan</span>
        </div>
      </div>
      ...
      <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-gray-100 flex items-center justify-between">
          <h3 className="text-lg font-black text-gray-900 flex items-center gap-2">
            ...
            <span>Proposal Pengajuan Masuk ({filtered.length})</span>
          </h3>
          <span className="text-xs text-gray-500 font-medium">Approval Guru Pembina</span>
        </div>
          ...
          <div className="bg-white w-full max-w-2xl rounded-3xl p-6 sm:p-8 space-y-6 max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-4 text-sm text-gray-700">
              ...
              <div>
                ...
                <textarea
                  rows={3}
                  placeholder="Tambahkan instruksi revisi atau catatan persetujuan untuk panitia..."
                  value={teacherNote}
            ...
            <div className="pt-2 flex flex-wrap items-center justify-end gap-2.5">
              <button
                onClick={() => handleUpdateStatus(selectedProposal.id, "Minta Revisi")}
              ...
              <button
                onClick={() => handleUpdateStatus(selectedProposal.id, "Tolak Proposal")}
              ...
              <button
                onClick={() => handleUpdateStatus(selectedProposal.id, "Disetujui Guru Pembina")}
